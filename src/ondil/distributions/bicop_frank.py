...
class BivariateCopulaFrank(BivariateCopulaMixin, CopulaMixin, Distribution):
    ...
    def pdf(self, y, theta):
        ...
        # Broadcast to (n, n) to match estimator interface like other copulas
        n = len(pdf_1d)
        pdf_2d = np.tile(pdf_1d.reshape(-1, 1), (1, n))
        return pdf_2d
    ...
    def dl1_dp1(self, y, theta, param=0):
        ...
        # Ensure theta_param is 1D to avoid shape issues
        theta_param = np.asarray(theta_param).flatten()
        deriv_1d = _derivative_1st(y, theta_param)
        # Broadcast to (n, n) to match estimator interface like other copulas
        n = len(deriv_1d)
        deriv_2d = np.tile(deriv_1d.reshape(-1, 1), (1, n))
        return deriv_2d
    ...
    def element_dl1_dp1(self, y, theta, param=0, k=0, clip=False):
        ...
        theta_param = np.asarray(theta_param).flatten()
        deriv_1d = _derivative_1st(y, theta_param)
        # Return (n, n) matrix as expected by estimator column extraction logic
        # This is the correct interface for multivariate copula derivatives
        n = len(deriv_1d)
        deriv_2d = np.tile(deriv_1d.reshape(-1, 1), (1, n))
        return deriv_2d
